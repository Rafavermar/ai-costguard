...
| Headroom | Disabled | Optional request compression via `headroom-ai` in library mode. Cost Guard calls `headroom.compress(messages, model=...)` when installed and enabled, records applied/skip evidence, exposes offline input-shape diagnostics, and keeps Headroom's conservative user-message defaults unless changed locally. Treat it as experimental on a workstation until real traffic shows positive Headroom savings. |
