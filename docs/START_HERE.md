...
For Headroom validation, first run `costguard headroom test --sample repeated` offline. If it returns `skipped_no_change`, compare `--input-shape messages-list`, `raw-text`, `openai-payload`, and `concatenated-messages-text` before spending more quota. For real traffic, look for `headroom_applied_count`, `headroom_tokens_saved`, and `headroom_reduction_ratio`. `outputs_reduced` means output truncation, not Headroom compression.
